...
class TopicsDetailMsg extends StatefulWidget {
  final data;
  const TopicsDetailMsg({Key? key, this.data}) : super(key: key);

  @override
  _TopicsDetailMsgState createState() => _TopicsDetailMsgState();
}

class _TopicsDetailMsgState extends State<TopicsDetailMsg> {
...
return  StreamBuilder(
                                        stream: FirebaseFirestore.instance
                                            .collection("unobilabs_tickets").where('t_topid',isEqualTo: '${widget.data['h_id']}').where('t_from',isEqualTo: '${UlStorage.read('mob')}').where('t_status',isEqualTo: 'Opened').snapshots(),
    ...
    elevation: 0,
    backgroundColor: Colors.white,
    title: Text('${widget.data['h_title']}',style: TextStyle(fontSize: 17.5,fontFamily: 'poppins',color: Colors.black),),
    centerTitle: false,
    leading: IconButton(
      onPressed: (){
                    ...
                    List added = await PostMethod('showTickets',
                        jsonEncode({
                          ...
                          if(snapshot.data!.docs.length != 0)
                            "docid": snapshot.data!.docs[0]['t_id'],
                          "receiver": "admin",
                          "top_id": "${widget.data['h_id']}",
                          "topic": "${widget.data['h_title']}",
                          "desc": "${widget.data['h_desc']}",
                          "status": "Opened",
