import 'dart:convert';

import 'package:flutter/material.dart';
import 'package:unilabs/Controllers/ApiCalls.dart';
import 'package:unilabs/Controllers/Constants.dart';
import 'package:unilabs/Models/Loading.dart';
import 'package:unilabs/Models/MedButton.dart';
import 'package:flutter_svg/flutter_svg.dart';

import 'package:unilabs/Models/ProductCard.dart';
import 'package:unilabs/Models/Theme.dart';
import 'package:unilabs/Views/Home/BasePage.dart';
import 'package:unilabs/Views/Home/ShippingDetails.dart';
import 'package:unilabs/Views/Home/mycartproductdetail.dart';


import '../../Models/HeadingText.dart';
import 'package:get_storage/get_storage.dart';

var noimg="";
final  UlStorage = GetStorage();
class MyCart extends StatefulWidget {
  const MyCart({Key? key}) : super(key: key);

  @override
  _MyCartState createState() => _MyCartState();
}

class _MyCartState extends State<MyCart> {
  List newbestdata=[];
  final UlStorage=GetStorage();
  final NormalUnobiStorage=GetStorage();

  newandbestcall() async{

    List respona=await GetMethod('showOrders/RHVR5A/${UlStorage.read('mob')}/all/Open');
    setState(() {
      newbestdata=respona;
    });
  }
  @override
  void initState() {

    newandbestcall();
    super.initState();

  }
  void dispose() {

    super.dispose();

  }
  @override
  Widget build(BuildContext context) {
print( UlStorage.read('pincode'));
print('pincodepincode');


    return Container(

      width: double.infinity,
      child: Column(
        children: [
          Center(
              child: HeadingPoppins(text: 'My Cart')),
          SizedBox(height: 30),
          newbestdata.length==0 ?
          loadkart(context):
          (newbestdata[0]['status']=='ok') ?
          Column(
            children: [

              for(int i=0; i<newbestdata[0]['products'].length; i++)
                if(newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty']>0)
                  InkWell(

                    child:  Container(
                      margin: EdgeInsets.only(bottom: 10),

                      width: MediaQuery.of(context).size.width*0.95,


                      decoration: BoxDecoration(
                        color: Colors.white,
                        borderRadius: BorderRadius.circular(12),
                        boxShadow: [
                          BoxShadow(
                            color:   Color.fromARGB(111, 150, 140, 140) ,
                            offset: const Offset(
                              1.0,
                              3.0,
                            ),
                            blurRadius:15.0,
                          ),         ],
                      ),
                      child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        child: Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                          children: [
                            Container(
                                width: MediaQuery.of(context).size.width*0.25,

                                child:newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_img'].length==0 ?
                                  Image.memory(
                                    Base64Codec().decode(noimg),
                                    fit: BoxFit.fill) : Image.network('https://unobilabs.com${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_img'][0]}', fit: BoxFit.fill)),
                            Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Row(
                                      children: [
                                        SizedBox(width: 0),
                                        Padding(
                                          padding: const EdgeInsets.all(8.0),
                                          child: Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                              mainAxisSize: MainAxisSize.max,
                                              children:[
                                                Column(
                                                  crossAxisAlignment: CrossAxisAlignment.start,
                                                  children: [
                                                    SizedBox(height: 5,),
                                                    Container(
                                                        width: 155,
                                                        margin: EdgeInsets.only(bottom: 5),
                                                        child: Text('${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_name'] }',style: TextStyle(fontFamily: 'cera medium',fontSize: 14,height: 1.2,color:UiColors.primary),overflow: TextOverflow.ellipsis,maxLines: 4,)),
                                                    Container(
                                                        width: 150,
                                                        child: Text('${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_category']}',style: TextStyle(fontSize: 12,color: Colors.grey,fontFamily: 'cera medium',),overflow: TextOverflow.ellipsis,))
                                                  ],
                                                ),


                                              ] ),
                                        ),
                                      ],
                                    ),
                                    SizedBox(width: 30,),

                                    Column(
                                      crossAxisAlignment: CrossAxisAlignment.end,
                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                      children: [

                                        InkWell(
                                          child: Icon(Icons.edit,size: 20,color: Colors.grey,),
                                          onTap: ()async{

                                            await  Navigator.of(context).push(MaterialPageRoute(builder: (context)=> MycartProductDetails(
                                              data: newbestdata,
                                              barcode: newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_barcode'],
                                              price:  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_salesprice'],
                                              qty:  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty'],
                                            )));
                                            setState(() {
                                              newbestdata=[];
                                              newandbestcall();
                                            });
                                          },
                                        ),

                                        SizedBox()
                                      ],
                                    ),

                                  ],
                                ),
                                Align(
                                  alignment: Alignment.bottomRight,
                                  child: Row(
                                    crossAxisAlignment: CrossAxisAlignment.end,
                                    mainAxisAlignment: MainAxisAlignment.start,
                                    children: [
                                      SizedBox(width: 8),

                                      Text('${ConstantsN.currency} ${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_mrp']} ',style: TextStyle(decoration: TextDecoration.lineThrough),),
                                      Text('${ConstantsN.currency}${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_salesprice']}',style: TextStyle(fontSize: 17.5,fontFamily: 'cera medium'),),
                                    ],
                                  ),
                                ),
                                Container(
                                  width: MediaQuery.of(context).size.width*0.56,

                                  child: Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [

                                      Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          SizedBox(height: 2,),
                                          Row(
                                            children: [
                                             // Text('Qty    : ',style: TextStyle(),),
                                              Text('  ${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty']}',style: TextStyle(fontFamily: 'cera medium',fontSize: 15.5),),
                                              Text(' X ${ConstantsN.currency}${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_salesprice']} = ',style: TextStyle(fontFamily: 'cera medium',fontSize: 15.5),textAlign: TextAlign.left,),
                                              Text('${ConstantsN.currency}${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty']* num.parse(newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_salesprice'])}',style: TextStyle(fontFamily: 'cera medium',fontSize: 15.5),textAlign: TextAlign.left,),
                                            ],
                                          ),
                                          Row(
                                            children: [
                                               ],
                                          ),
                                      
                                        ],
                                      ),
                                      InkWell(
                                        child:  SvgPicture.asset('assets/svgs/delete.svg',height: 23,width: 25,color: UiColors.primary,),
                                        onTap: () {
                                          showDialog(
                                            context: context,
                                            builder: (BuildContext context) {
                                              return AlertDialog(

                                                contentPadding: EdgeInsets.all(12),
                                                content: StatefulBuilder(
                                                    builder: (BuildContext context, StateSetter setState) {
                                                      return Padding(
                                                        padding: const EdgeInsets.all(8.0),
                                                        child: SafeArea(
                                                          child: Container(


                                                            margin: const EdgeInsets.all(8.0),

                                                            child:  Column(
                                                              mainAxisSize: MainAxisSize.min,
                                                              children: [
                                                                SizedBox(height: 10,),
                                                                Row(crossAxisAlignment: CrossAxisAlignment.center,
                                                                  mainAxisAlignment: MainAxisAlignment.center,
                                                                  children: [
                                                                    Text('Are you sure, \nYou want to remove this item?',style: TextStyle(fontFamily: 'poppins medium'),textAlign: TextAlign.center,),

                                                                  ],
                                                                ),
                                                                SizedBox(height: 30,),
                                                                Row(
                                                                  mainAxisAlignment: MainAxisAlignment.center,
                                                                  children: [
                                                                    UiCancelButtom(text: ' Cancel ', ontap: (){
                                                                      Closepop(context);
                                                                    },),
                                                                    SizedBox(width: 15,),
                                                                    UiButtonSec(text: 'Remove ', ontap: ()async{

                                                                      showDialog(
                                                                          barrierColor: Colors.black38,
                                                                          barrierDismissible: false,
                                                                          context: context,
                                                                          builder: (BuildContext context){
                                                                            return Loading(title: "Removing",);
                                                                          }
                                                                      );

                                                                      List added=await PostMethod('showOrders',
                                                                          jsonEncode({
                                                                            "code" : "RHVR5A",
                                                                            "phone" : UlStorage.read('mob'),
                                                                            "barocde" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_barcode'],
                                                                            "p_name" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_name'],
                                                                            "p_cgst" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_cgst'],
                                                                            "p_sgst" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_sgst'],
                                                                            "p_mrp" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_mrp'],
                                                                            "p_salesprice" : newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_salesprice'],
                                                                            "p_qty" :0,
                                                                            "p_img" : newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_img'],
                                                                            "category" :  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_category'],
                                                                            "p_desc" :  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_desc'],
                                                                          }));

                                                                      if(added[0]['status']=='ok'){

                                                                        Navigator.of(context).pop(null);
                                                                        Navigator.of(context).pop(null);

                                                                        successtoast("Successfully removed");
                                                                        newbestdata=[];
                                                                        newandbestcall()  ;
                                                                      }
                                                                      else{
                                                                        Navigator.of(context).pop(null);
                                                                        Navigator.of(context).pop(null);
                                                                        errortoast("Failed to remove");
                                                                      }

                                                                    }),



                                                                  ],
                                                                ),
                                                                SizedBox(height: 10,),


                                                              ],
                                                            ),
                                                          ),
                                                        ),
                                                      );

                                                    }),
                                              );
                                            },
                                          );
                                        },
                                      ),
                                    ],
                                  ),
                                ),

                              ],
                            )

                          ],
                        ),
                      ),

                    ),
                  ),

              SizedBox(height: 30),
              newbestdata[0]['b_total']!='0.00'?
              Column(
                children: [
                  Container(



                    decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(10),
                        color: Colors.white
                    ),
                    child: Padding(
                      padding: const EdgeInsets.all(8.0),
                      child:  Column(
                        children: [

                          Align(
                            alignment: Alignment.topLeft,
                            child:Text("Summary",style: TextStyle(fontSize: 18,color: UiColors.primary)), ),
                          SizedBox(height: 5,),
                          Padding(
                            padding: const EdgeInsets.all(6.0),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text('Sub total'),
                                Text('${ConstantsN.currency} ${newbestdata[0]['b_mrp']}',style:TextStyle(fontFamily: 'cera medium')),
                              ],
                            ),
                          ),
                          Padding(
                            padding: const EdgeInsets.all(6.0),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text('Discount '),
                                Text('${ConstantsN.currency} ${(num.parse(newbestdata[0]['b_mrp']) - num.parse(newbestdata[0]['b_total'])).toStringAsFixed(2)}',style:TextStyle(fontFamily: 'cera medium',color: Colors.red,decoration: TextDecoration.lineThrough),),
                              ],
                            ),
                          ),
                          Padding(
                            padding: const EdgeInsets.all(6.0),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text('Shipping charge'),
                                Text( newbestdata[0]['shipping'] ,style:TextStyle(fontFamily: 'cera medium',color: Colors.red)),
                              ],
                            ),
                          ),

                          Divider(),
                          NormalUnobiStorage.read('CountryName')== 'IN'?  Padding(
                            padding: const EdgeInsets.all(6.0),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text('CGST'),
                                Text(" ${ConstantsN.currency} ${newbestdata[0]['b_cgst_total']}",style:TextStyle(fontFamily: 'cera medium')),
                              ],
                            ),
                          ) : SizedBox(),
                          NormalUnobiStorage.read('CountryName')== 'IN'? Padding(
                            padding: const EdgeInsets.all(6.0),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text('SGST'),
                                Text("${ConstantsN.currency} ${newbestdata[0]['b_sgst_total']}",style:TextStyle(fontFamily: 'cera medium')),
                              ],
                            ),
                          ):  SizedBox(),
                          Padding(
                            padding: const EdgeInsets.all(6.0),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text('Total Tax (Included)'),
                                Text("${ConstantsN.currency} ${newbestdata[0]['b_gst_total']}",style:TextStyle(fontFamily: 'cera medium')),
                              ],
                            ),
                          ),

                          Divider(),
                          Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text('Total',style:TextStyle(fontFamily: 'cera medium',fontSize: 15)),
                                Text('${ConstantsN.currency} ${newbestdata[0]['b_total']}',style:TextStyle(fontFamily: 'cera medium',fontSize: 15)),
                              ],
                            ),
                          ),
                        ],
                      ),
                    ),

                  ),
                  SizedBox(height: 30),
                  newbestdata[0]['b_total']!=0 ? UiButton(text: 'Checkout', ontap: ()
                  async{

                    Navigator.of(context).push(MaterialPageRoute(builder: (context)=>
                        ShippingDetails(
                          upi:num.parse( newbestdata[0]['discount_card']),
                          creditcard: num.parse(newbestdata[0]['discount_upi']),
                          shipping: newbestdata[0]['shipping'],
                          cop:num.parse(newbestdata[0]['discount']),
                          payid:newbestdata[0]['upi_id'],
                          data: newbestdata[0],
                          billno: newbestdata[0]["bill_number"],
                          tot: num.parse(newbestdata[0]["b_total"]).round(),)));
                  }) :
                  UiButton(text: 'Checkout', ontap: (){}),

                ],
              )
                  :  Column(
                children: [
                  SizedBox(height:85),

                  Padding(
                    padding: const EdgeInsets.only(right: 25),
                    child: SvgPicture.asset('assets/svgs/emptycart.svg'),
                  ),
                  SizedBox(height:25),

                  Text("Your Cart is Empty",style: TextStyle(fontSize: 23,fontFamily: 'cera medium'),),
                  SizedBox(height:2),
                  Text("Looks like you haven't added \nanything to your cart yet",style: TextStyle(fontSize: 17,fontFamily: 'cera'),textAlign: TextAlign.center,),
                  SizedBox(height:50),
                  UiButton(text: 'Start Shopping', ontap: (){
                    Navigator.of(context).push(MaterialPageRoute(builder: (context)=> BaseScreen(toScreen: 'category',)));

                  })

                ],
              ),


            ],
          )

              :  Column(
            children: [
              SizedBox(height:125),

              Padding(
                padding: const EdgeInsets.only(right: 25),
                child: SvgPicture.asset('assets/svgs/emptycart.svg'),
              ),
              SizedBox(height:25),

              Text("Your Cart is Empty",style: TextStyle(fontSize: 23,fontFamily: 'cera medium'),),
              SizedBox(height:2),
              Text("Looks like you haven't added \nanything to your cart yet",style: TextStyle(fontSize: 17,fontFamily: 'cera'),textAlign: TextAlign.center,),
              SizedBox(height:50),
              UiButton(text: 'Start Shopping', ontap: (){
                Navigator.of(context).push(MaterialPageRoute(builder: (context)=> BaseScreen(toScreen: 'category',)));

              })

            ],
          ),
          SizedBox(height: 30),

        ],

      ),
    );
  }

}
