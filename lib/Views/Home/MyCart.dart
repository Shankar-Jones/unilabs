import 'dart:convert';

import 'package:flutter/material.dart';
import 'package:onshop/Controllers/ApiCalls.dart';
import 'package:onshop/Models/Loading.dart';
import 'package:onshop/Models/MedButton.dart';
import 'package:flutter_svg/flutter_svg.dart';

import 'package:onshop/Models/ProductCard.dart';
import 'package:onshop/Models/Theme.dart';
import 'package:onshop/Views/Home/ShippingDetails.dart';


import '../../Models/HeadingText.dart';
import 'package:get_storage/get_storage.dart';

var noimg="";

class MyCart extends StatefulWidget {
  const MyCart({Key? key}) : super(key: key);

  @override
  _MyCartState createState() => _MyCartState();
}

class _MyCartState extends State<MyCart> {
    List newbestdata=[];
  final ShopStorage=GetStorage();
  newandbestcall() async{
   
    List respona=await GetMethod('showOrders/RH46IB/${ShopStorage.read('mob')}/all/Open');
    setState(() {
      newbestdata=respona;
    });
  }
   @override
  void initState() {
   
   newandbestcall();
  super.initState();

}
void dispose() {

  super.dispose();

}
  @override
  Widget build(BuildContext context) {
    print(newbestdata);
    return Container(
      height: MediaQuery.of(context).size.height,
      width: double.infinity,
      child: Column(
        children: [
          Center(
              child: HeadingPoppins(text: 'My Cart')),
          SizedBox(height: 30),
          newbestdata.length==0 ?
          loadkart(context):
           (newbestdata[0]['status']=='ok') ?
          Column(
            children: [

              for(int i=0; i<newbestdata[0]['products'].length; i++)
              if(newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty']>0)
              Container(
                margin: EdgeInsets.only(bottom: 10),
                height: 120,
                width: MediaQuery.of(context).size.width*0.90,

                decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(10),
                    color: UiColors.primaryShade
                ),
                child: Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Row(
                        children: [
                          Container(
                              width: MediaQuery.of(context).size.width*0.28,

                              child: Image.memory(
                                 newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_img'].length==0 ?
                              Base64Codec().decode(noimg) :  Base64Codec().decode(newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_img'][0]) ,

                              fit: BoxFit.fill)),
                          SizedBox(width: 0),
                          Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                mainAxisSize: MainAxisSize.max,
                                children:[
                                  Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      SizedBox(height: 5,),
                                      Container(
                                          width: 105,
                                          child: Text('${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_name'] }',style: TextStyle(fontFamily: 'poppins bold',fontSize: 17),overflow: TextOverflow.ellipsis,maxLines: 2,)),
                                      Text(newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_category'],style: TextStyle(fontSize: 13,color: Colors.grey,fontFamily: 'archivo bold',),)
                                    ],
                                  ),

                                  Text('\u{20B9} ${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_salesprice']}',style: TextStyle(fontFamily: 'archivo bold',fontSize: 17),),

                                ] ),
                          ),
                        ],
                      ),
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.end,
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          InkWell(
                            child: Icon(
                              Icons.highlight_off_outlined,
                            ),
                            onTap: () {
                              showDialog(
                                context: context,
                                builder: (BuildContext context) {
                                  return AlertDialog(

                                    contentPadding: EdgeInsets.all(12),
                                    content: StatefulBuilder(
                                        builder: (BuildContext context, StateSetter setState) {
                                          return Padding(
                                            padding: const EdgeInsets.all(8.0),
                                            child: SafeArea(
                                              child: Container(


                                                margin: const EdgeInsets.all(8.0),

                                                child:  Column(
                                                  mainAxisSize: MainAxisSize.min,
                                                  children: [
                                                    SizedBox(height: 10,),
                                                    Row(crossAxisAlignment: CrossAxisAlignment.center,
                                                      mainAxisAlignment: MainAxisAlignment.center,
                                                      children: [
                                                        Text('Are you sure, \nYou want to remove this item?',style: TextStyle(fontFamily: 'poppins medium'),textAlign: TextAlign.center,),

                                                      ],
                                                    ),
                                                    SizedBox(height: 30,),
                                                    Row(
                                                      mainAxisAlignment: MainAxisAlignment.center,
                                                      children: [
                                                        UiCancelButtom(text: ' Cancel ', ontap: (){
                                                          Closepop(context);
                                                        },),
                                                        SizedBox(width: 15,),
                                                        UiButtonSec(text: 'Remove ', ontap: ()async{

                                                               showDialog(
                            barrierColor: Colors.black38,
                            barrierDismissible: false,
                            context: context,
                            builder: (BuildContext context){
                              return Loading(title: "Removing",);
                            }
                        );

                         List added=await PostMethod('showOrders',
          jsonEncode({
              "code" : "RH46IB",
              "phone" : ShopStorage.read('mob'),
              "barocde" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_barcode'],
              "p_name" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_name'],
              "p_cgst" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_cgst'],
              "p_sgst" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_sgst'],
              "p_mrp" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_mrp'],
              "p_salesprice" : newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_salesprice'],
              "p_qty" :0,
              "p_img" : newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_img'],
              "category" :  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_category'],
              "p_desc" :  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_desc'],
            }));

            if(added[0]['status']=='ok'){

                Navigator.of(context).pop(null);
                 Navigator.of(context).pop(null);

              successtoast("Removed Successfully");
                newbestdata=[];
                newandbestcall()  ;
            }
            else{
                   Navigator.of(context).pop(null);
                    Navigator.of(context).pop(null);
              errortoast("Failed to remove");
            }

                                                        }),



                                                      ],
                                                    ),
                                                    SizedBox(height: 10,),


                                                  ],
                                                ),
                                              ),
                                            ),
                                          );

                                        }),
                                  );
                                },
                              );
                          },
                          ),
                          Container(
                              alignment: Alignment.centerRight,
                              decoration: BoxDecoration(

                                  color: UiColors.primarySec,
                                  borderRadius: BorderRadius.circular(100)
                              ),
                              child: Padding(
                                padding: const EdgeInsets.symmetric(vertical: 5.0,horizontal: 6),
                                child: Row(
                                  children: [
                                    InkWell(
                                    onTap:() async{
                                            if(newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty']>1){
                                              setState(() {
                                                newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty']=newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty']-1;
                                              });
                                               List added=await PostMethod('showOrders',
          jsonEncode({
              "code" : "RH46IB",
              "phone" : ShopStorage.read('mob'),
              "barocde" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_barcode'],
              "p_name" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_name'],
              "p_cgst" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_cgst'],
              "p_sgst" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_sgst'],
              "p_mrp" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_mrp'],
              "p_salesprice" : newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_salesprice'],
              "p_qty" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty'],
              "p_img" :  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_img'],
              "category" :  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_category'],
              "p_desc" : newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_desc'],
            }));

                                            }
                                          },
                                    child:Container(
                                        decoration: BoxDecoration(
                                            shape: BoxShape.circle,
                                            color: Colors.white
                                        ),
                                        child:   Padding(
                                            padding: const EdgeInsets.all(3.0),
                                            child: Icon(Icons.remove,color: Colors.black,size: 15,),
                                          ),

                                    ),),
                                    SizedBox(width: 15,),
                                    Text('${newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty']}',style: TextStyle(fontFamily: 'Archivo bold',fontSize: 13),),
                                    SizedBox(width: 15,),
                                    InkWell(
                                      onTap: () async{

                                        showDialog(
                                            barrierColor: Colors.black38,
                                            barrierDismissible: false,
                                            context: context,
                                            builder: (BuildContext context){
                                              return Loading(title: "adding",);
                                            }
                                        );


                                                   List added=await PostMethod('showOrders',
          jsonEncode({
              "code" : "RH46IB",
              "phone" : ShopStorage.read('mob'),
              "barocde" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_barcode'],
              "p_name" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_name'],
              "p_cgst" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_cgst'],
              "p_sgst" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_sgst'],
              "p_mrp" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_mrp'],
              "p_salesprice" : newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_salesprice'],
              "p_qty" :newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_qty']+1,
              "p_img" :  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_img'],
              "category" :  newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_category'],
              "p_desc" : newbestdata[0]['products']['${newbestdata[0]["b_products"][i]}']['p_desc'],
            }));
                                                   newbestdata=[];
                                                    newandbestcall();
                                                   Navigator.pop(context);



                                      },
                                      child: Container(

                                          decoration: BoxDecoration(
                                              shape: BoxShape.circle,
                                              color: Colors.white
                                          ),
                                          child: Padding(
                                            padding: const EdgeInsets.all(3.0),
                                            child: Icon(Icons.add,color: Colors.black,size: 15,),
                                          )
                                      ),
                                    ),
                                  ],
                                ),
                              )
                          ),
                        ],
                      )

                    ],
                  ),
                ),

              ),

            SizedBox(height: 30),
            Container(



            decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(10),
                color: UiColors.primaryShade
            ),
            child: Padding(
              padding: const EdgeInsets.all(8.0),
              child:  Column(
                children: [
                  Padding(
                    padding: const EdgeInsets.all(6.0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                          Text('Subtotal'),
                          Text('\u{20B9} ${newbestdata[0]['b_mrp']}',style:TextStyle(fontFamily: 'archivo bold')),
                      ],
                    ),
                  ),
                  Padding(
                    padding: const EdgeInsets.all(6.0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                          Text('Shipping charge'),
                          Text('Free',style:TextStyle(fontFamily: 'archivo bold',color: Colors.red)),
                      ],
                    ),
                  ),

                  Divider(),
                  Padding(
                    padding: const EdgeInsets.all(6.0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('Total Tax'),
                        Text("${newbestdata[0]['b_gst_total']}",style:TextStyle(fontFamily: 'archivo bold')),
                      ],
                    ),
                  ),

                  Divider(),
                  Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('Total',style:TextStyle(fontFamily: 'archivo bold',fontSize: 15)),
                        Text('\u{20B9} ${newbestdata[0]['b_total']}',style:TextStyle(fontFamily: 'archivo bold',fontSize: 15)),
                      ],
                    ),
                  ),
                ],
              ),
            ),

          ),

          SizedBox(height: 30),
          UiButton(text: 'Checkout', ontap: ()
          async{
               Navigator.of(context).push(MaterialPageRoute(builder: (context)=> ShippingDetails(
                billno: newbestdata[0]["bill_number"],
                tot: num.parse(newbestdata[0]["b_total"]).round(),)));
          }),
            ],
          )

         :  Column(
           children: [
             SizedBox(height:125),

             Padding(
               padding: const EdgeInsets.only(right: 25),
               child: SvgPicture.asset('assets/svgs/emptycart.svg'),
             ),
             SizedBox(height:25),

             Text("Your Cart is Empty",style: TextStyle(fontSize: 23,fontFamily: 'poppins'),),
             SizedBox(height:2),
             Text("Looks like you haven't added \nanything to your cart yet",style: TextStyle(fontSize: 18,fontFamily: 'poppins medium'),textAlign: TextAlign.center,),
             SizedBox(height:50),
             UiButton(text: 'Start Shopping', ontap: (){})

           ],
         ),
          SizedBox(height: 30),

        ],

      ),
    );
  }

}
