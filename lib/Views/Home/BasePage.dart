import 'dart:convert';
import 'package:flutter_spinkit/flutter_spinkit.dart';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:flutter/rendering.dart';
import 'package:flutter_svg/flutter_svg.dart';
import 'package:unilabs/Controllers/ApiCalls.dart';
import 'package:unilabs/Models/HeadingText.dart';
import 'package:unilabs/Models/ProductCard.dart';
import 'package:unilabs/Models/Theme.dart';
import 'package:unilabs/Views/Home/BestSelling.dart';
import 'package:unilabs/Views/Home/CategoryWise_Products.dart';
import 'package:unilabs/Views/Home/MyAccount.dart';
import 'package:unilabs/Views/Home/MyCart.dart';
import 'package:unilabs/Views/Home/MyFavourites.dart';
import 'package:unilabs/Views/Home/NewArrivals.dart';
import 'package:unilabs/Views/Home/TrackingOrders.dart';
import 'package:carousel_slider/carousel_slider.dart';
import '../../Models/MyCoins.dart';
import 'ProductDetails.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:shimmer/shimmer.dart';
import 'package:get_storage/get_storage.dart';

import 'Search_Product.dart';
import 'Searched_Product_Results.dart';
import 'UnoCoins_Activities.dart';
var noimg="";
 String? ActivePage;
String category="All";
bool favorite = false;
final UlStorage=GetStorage();
final AppTheme=GetStorage();
class BaseScreen extends StatefulWidget {
  final fromWhere;
  final toScreen;
  const BaseScreen({Key? key, this.fromWhere, this.toScreen}) : super(key: key);

  @override
  _BaseScreenState createState() => _BaseScreenState();
}

class _BaseScreenState extends State<BaseScreen> {
 
   List catlist=[];
   List slidelist=[];
   List newbestdata=[];
   TextEditingController NameController = TextEditingController();

   categorygetcall() async{
   
    List respon=await GetMethod('addCategory/RHVR5A');
    setState(() {
      catlist=respon;
    });
  }
  Slidercall() async{
   
     List respon=await GetMethod('addBanners/RHVR5A');
    setState(() {
      slidelist=respon;
    });
  }
newandbestcall() async{
   
    List respona=await GetMethod('showNewArrivals/RHVR5A');
    setState(() {
      newbestdata=respona;
    });
  }
  @override
  void initState() {

    NameController.clear();
    keywrds='';
    if(widget.toScreen == ''){
      setState(() {
        ActivePage='home';
      });}
    if(widget.toScreen == 'cart'){
      setState(() {
        ActivePage='mycart';
      });}
    if(widget.toScreen=='category'){
      setState(() {
        ActivePage='category';
      });

    }
    if(widget.toScreen=='favourite'){
      setState(() {
        ActivePage='favourite';
      });
    }
    if(widget.toScreen=='profile'){
      setState(() {
        ActivePage='profile';
      });
    }
    if(widget.fromWhere == 'Shipping Details'){
      setState(() {
        ActivePage = 'home';
      });
    }
    if(widget.fromWhere == 'profile'){
      setState(() {
        ActivePage = 'profile';
      });
    }
   categorygetcall();
   newandbestcall();
   Slidercall();

  super.initState();

}
void dispose() {
NameController.dispose();
  super.dispose();

}
  @override
  Widget build(BuildContext context) {
    return SafeArea(
        child: Scaffold(
          backgroundColor: Colors.white,
         body: SingleChildScrollView(
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 5.0),
            child: Column(
              children: [
                     SizedBox(height:20),
                if(ActivePage=='home')
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 12.0),

                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [

                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Row(
                            children: [
                              InkWell(
                                onTap: (){
                                  setState(() {
                                    ActivePage ='profile';
                                    keywrds='';
                                    NameController.clear();
                                  });
                                },
                                child: CircleAvatar(
                                  radius: 28,backgroundColor: Colors.transparent,

                                  foregroundImage:  MemoryImage(Base64Codec().decode(UlStorage.read('profile'))
                                  ),
                                ),
                              ),
                              SizedBox(width: 10,),
                              Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Row(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text('Hi,',style: TextStyle(color: UiColors.primary,fontSize: 16, )),
                                      InkWell(
                                          onTap: (){
                                         //   Navigator.of(context).push(MaterialPageRoute(builder: (context)=> PersonalDetailsPage()));

                                          },
                                          child: Text(UlStorage.read('u_name'),style: TextStyle(color: UiColors.primary,fontSize: 17,fontFamily: 'cera medium'),)),
                                    ],
                                  ),
                                  SizedBox(height: 5,),
                                  InkWell(
                                      onTap: (){
                                        Navigator.of(context).push(
                                            MaterialPageRoute(builder: (context)=> UnoCoinsActivities())
                                        );
                                      },
                                      child: MyCoins())

                                ],
                              )
                            ],
                          ),
                          InkWell(
                            onTap: (){
                                   Navigator.of(context).push(MaterialPageRoute(builder: (context)=> TrackingOrders()));
                                   keywrds='';
                                   NameController.clear();
                            },
                            child: SvgPicture.asset('assets/svgs/tracking.svg',height: 30,)),
                        ],
                      ),
                      SizedBox(height:15),


                      Padding(
                        padding: const EdgeInsets.only(bottom: 10,top: 7),
                        child: InkWell(
                          onTap: (){
                          // Navigator.of(context).push(MaterialPageRoute(builder: (context)=> SearchProduct()));

                          },
                          child: Stack(
                            children: [
                              Container(
                                decoration: BoxDecoration(
                                    color: Color(0xfff1f2f5),
                                    borderRadius: BorderRadius.circular(9),
                                    border: Border()
                                ),
                                child: Center(
                                  child: Padding(
                                    padding: const EdgeInsets.only(left: 15),
                                    child: TextField(

                                      controller: NameController,

                                      onChanged:(val){
                                        setState(() {
                                          keywrds=val;
                                        });

                                      },

                                      style: TextStyle(fontSize: 18,),
                                      decoration: InputDecoration(
                                        suffixIcon: Padding(
                                          padding: const EdgeInsets.only(right: 0),
                                          child: IconButton(icon: Icon(Icons.close,color:NameController.text.length!=0 ? Colors.grey : Color(0xfff1f2f5),size: 20,),onPressed: (){
                                            setState(() {

                                            NameController.clear();

                                            });
                                          }),
                                        ),
                                        hintText: 'Search products...',
                                        hintStyle: TextStyle(color:UiColors.primarySec,fontSize: 16,fontFamily: 'cera medium'),
                                        border: InputBorder.none,
                                      ),
                                    ),
                                  ),
                                ),
                              ),
                              if(NameController.text.length!=0)
                              Container(
                                margin: EdgeInsets.only(top: 45),
                                  width: double.infinity,

                                decoration: BoxDecoration(
                                  color: Colors.white,
                                  border: Border(bottom: BorderSide(color: Colors.black45,width: 0.6),right: BorderSide(color: Colors.black,width: 0.6),left: BorderSide(color: Colors.black,width: 0.6),),
                                /*  boxShadow: [
                                    BoxShadow(
                                      color:  UiColors.Shadow_Clr2,

                                      offset: const Offset(
                                        0.0,
                                        0.0,
                                      ),
                                      blurRadius: 5.0,
                                    ),
                                  ],*/

                                ),
                                child: Column(
                                  children: [
                                    StreamBuilder(
                                        stream: FirebaseFirestore.instance
                                            .collection("unobi_companies").doc('${UlStorage.read('comcode')}').collection('keywords')
                                            .snapshots(),
                                        builder: (BuildContext context,
                                            AsyncSnapshot<QuerySnapshot> snapshot) {
                                          if(snapshot.hasData) {

                                            return StatefulBuilder(
                                                builder: (BuildContext context, StateSetter setState) {
                                                  return Column(
                                                    children: [
                                                      if(NameController.text.length!=0)
                                                        Container(
                                                          height: 165,
                                                          child: Scrollbar(isAlwaysShown: true,
                                                            child: SingleChildScrollView(
                                                              child: Column(
                                                                children: [
                                                                if(NameController.text.length>2)
                                                              for(int k=0; k<snapshot.data!.docs[0]['p_keywords'].length; k++)
                                                                 if(snapshot.data!.docs[0]['p_keywords'][k].toLowerCase().contains(keywrds.toLowerCase()))
                                                                    Column(
                                                                        children: [
                                                                          InkWell(
                                                                            onTap: (){
                                                                              Navigator.of(context).push(MaterialPageRoute(builder: (context)=> SearchedProductResults(data: snapshot.data!.docs[0]['p_keywords'][k],)));
                                                                              keywrds='';
                                                                              NameController.clear();
                                                                            },
                                                                            child: Container(

                                                                              decoration: BoxDecoration(
                                                                                color: Colors.white,
                                                                                boxShadow: [
                                                                                  BoxShadow(
                                                                                    color:AppTheme.read('mode')=="0" ?   UiColors.Shadow_Clr : Colors.transparent,
                                                                                    offset: const Offset(
                                                                                      1.0,
                                                                                      3.0,
                                                                                    ),
                                                                                    blurRadius:15.0,
                                                                                  ),         ],
                                                                              ),
                                                                              child: Center(
                                                                                child: Padding(
                                                                                  padding: const EdgeInsets.symmetric(vertical: 12.0,horizontal: 7),
                                                                                  child: Row(
                                                                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                                                                    children: [
                                                                                      Container(
                                                                                          width:MediaQuery.of(context).size.width-80,
                                                                                          child: Text(snapshot.data!.docs[0]['p_keywords'][k],textAlign: TextAlign.left,style: TextStyle(fontFamily: 'poppins medium',fontSize: 12),overflow:  TextOverflow.clip ,)),
                                                                                      Icon(Icons.north_east_outlined,size: 15,color: Colors.grey,)
                                                                                    ],
                                                                                  ),
                                                                                ),
                                                                              ),
                                                                            ),
                                                                          ) ,

                                                                        ],
                                                                      ),
                                                               Column(
                                                                 mainAxisAlignment: MainAxisAlignment.center,
                                                                 children: [
                                                                   SizedBox(height: 70,),
                                                                   Center(child: Text('No more products available',style:TextStyle( fontSize: 15,fontFamily: 'cera medium'),)),
                                                                 ],
                                                               ),
                                                               /*   Align(
                                                                      alignment: Alignment.centerRight,
                                                                      child: Padding(
                                                                        padding: const EdgeInsets.symmetric(vertical: 10,horizontal: 15),
                                                                        child: InkWell(onTap: (){
                                                                          Navigator.of(context).push(MaterialPageRoute(builder: (context)=> SearchProduct()));
                                                                        }, child: Row(
                                                                          mainAxisAlignment: MainAxisAlignment.end,
                                                                          children: [
                                                                            Text('see all',style: TextStyle(fontFamily:"cera medium",color: Colors.blue,fontSize: 15 ),),
                                                                            Icon(
                                                                              Icons.chevron_right_outlined,color: Colors.blue,size: 20,
                                                                            ),
                                                                          ],
                                                                        )),
                                                                      )),*/
                                                                ],
                                                              ),
                                                            ),
                                                          ),
                                                        ),

                                                    ],
                                                  );
                                                });
                                          }
                                          else {
                                            return Align(
                                              alignment: Alignment.center,
                                              child: Column(
                                                  children: [
                                                    SizedBox(height: 20,),
                                                    Padding(
                                                        padding:EdgeInsets.fromLTRB(0,0,0,0),
                                                        child: SpinKitHourGlass(
                                                          color: UiColors.gradient1,size: 40,)


                                                    )]

                                              ),
                                            );
                                          }
                                        }),
                                  ],
                                ),
                              )
                            ],
                          ),
                        ),
                      ),
                      SizedBox(height:0),

                      slidelist.length==0?
                      loadslider():
                      (slidelist[0]['status']=='ok' ?
                      CarouselSlider(
                        items: [
                          for(int p=0;p<slidelist.length;p++)

                            Container(

                              margin: EdgeInsets.all(6.0),
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(8.0),
                                image: DecorationImage(
                                  image: MemoryImage(Base64Codec().decode(slidelist[p]['b_img'])),
                                  fit: BoxFit.cover,
                                ),
                              ),
                            ),


                        ],

                        //Slider Container properties
                        options: CarouselOptions(
                          height: 150.0,
                          enlargeCenterPage: true,
                          autoPlay: true,
                          aspectRatio: 16 / 9,
                          autoPlayCurve: Curves.fastOutSlowIn,
                          enableInfiniteScroll: true,
                          autoPlayAnimationDuration: Duration(milliseconds: 800),
                          viewportFraction: 0.8,
                        ),
                      ): Text('No sliders available at this moment')),
                    SizedBox(height:15),



                      Padding(
                        padding: const EdgeInsets.only(top:10,bottom: 5),
                        child:catlist.length==0 ?
                        loadcategorytabbutton() : Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Categories',style: TextStyle(color: UiColors.primary,fontSize: 17,fontFamily: 'cera medium'),),

                            Container(
                              width: double.infinity,
                              child: SingleChildScrollView(
                                scrollDirection: Axis.horizontal,
                                child:


                                Row(
                                  children: [

                                    for(int g=0;g<catlist.length;g++)

                                      GestureDetector(
                                        onTap: (){
                                          setState(() {
                                            category=catlist[g]['c_name'];
                                          });
                                          Navigator.of(context).push(MaterialPageRoute(builder: (context)=> cateofproduct(catname: catlist[g]['c_name'],)));
                                          keywrds='';
                                          NameController.clear();
                                        },
                                        child:Padding(
                                          padding: const EdgeInsets.only(bottom: 8.0),
                                          child: CategoryProductCard(
                                              ImageUrl: Base64Codec().decode(catlist[g]['c_img']),
                                              startingRate: '200',
                                              itemsCount: '200',
                                              productName:catlist[g]['c_name']
                                          ),
                                        ),
                                      ),



                                  ],
                                ),
                              ),
                            ),
                          ],
                        ),
                      ),


                      newbestdata.length==0 ?
                       Column(
                        children: [
                            Loadcategoryofproduct(),
                             Loadcategoryofproduct(),
                        ]):
                      Column(
                        children: [
                          SizedBox(height: 15,),
                              Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text('New Arrivals',style: TextStyle(color: UiColors.primary,fontSize: 17,fontFamily: 'cera medium'),),

                          InkWell(
                            onTap: (){
                                    Navigator.of(context).push(MaterialPageRoute(builder: (context)=> NewArrivals()));
                                    keywrds='';
                                    NameController.clear();
                            },
                            child: Row(
                              children: [
                                Text('Show all',style: TextStyle(color: Colors.black,fontFamily: 'cera medium'),),
                                Icon(Icons.keyboard_arrow_right_outlined,color: Colors.black,size: 23,)
                              ],
                            ),
                          )
                        ],
                      ),


                          newbestdata.length==0 ? Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Text('No New Arrivals'),
                          ) :
                      SingleChildScrollView(
                        scrollDirection: Axis.horizontal,
                        physics: BouncingScrollPhysics(),
                        child: Row(
                          children: [

                            for(int k=0;k<newbestdata.length;k++)
                            if(newbestdata[k]['type']=='new')

                            ProductCard(
                              ontap:() async{
                                Navigator.of(context).push(MaterialPageRoute(builder: (context)=> ProductDetails(data: newbestdata[k],frmwhr: '',packSize: newbestdata[k]["p_other_desc"]['p_pack'],)));
                                keywrds='';
                                NameController.clear();
                              },
                             iontap: () async{
                                  if(newbestdata[k]['p_fav'].contains(UlStorage.read('mob'))){
                                     setState(() {
                              newbestdata[k]['p_fav'].remove(UlStorage.read('mob'));
                            });
                                    List res=await PutMethod('showOrders/RHVR5A/${newbestdata[k]['p_barcode']}',
                                    jsonEncode({
                               "type" : "remove",
                               "phone" : UlStorage.read('mob')
                            }));

                                  }
                                  else{
                                       setState(() {
                              newbestdata[k]['p_fav'].add(UlStorage.read('mob'));
                            });
                      List resa=await PutMethod('showOrders/RHVR5A/${newbestdata[k]['p_barcode']}',
                                    jsonEncode({
                               "type" : "add",
                               "phone" : UlStorage.read('mob')
                            }));

                                  }
                              },
                              ImageUrl: newbestdata[k]['p_img'].length==0 ?
                              Base64Codec().decode(noimg) : newbestdata[k]['p_img'][0] ,
                              productName: newbestdata[k]['p_name'],productPercentage:newbestdata[k]['p_type'] ? '': newbestdata[k]['p_discount'],
                              icon:newbestdata[k]['p_fav'].contains(UlStorage.read('mob')),
                               mrp: newbestdata[k]['p_mrp'],
                              productRate:
                              newbestdata[k]['p_type'] ?
                              newbestdata[k]['p_sellingprice']["0"]['p_price'] :
                              newbestdata[k]['p_sellingprice'], brandName: newbestdata[k]['p_brand'], packSize: newbestdata[k]["p_other_desc"]['p_pack'],),


                          ],
                        ),
                      ),

 InkWell(
                            onTap: (){
                                    Navigator.of(context).push(MaterialPageRoute(builder: (context)=> BestSelling()));
                                    keywrds='';
                                    NameController.clear();
                            },
                            child:
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text('Best Selling',style: TextStyle(color: UiColors.primary,fontSize: 17,fontFamily: 'cera medium'),),

                          Row(
                            children: [
                              Text('Show all',style: TextStyle(color: Colors.black,),),
                              Icon(Icons.keyboard_arrow_right_outlined,color: Colors.black,)
                            ],
                          )
                        ],
                      ),),


                      newbestdata.length==0 ? Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Text('No Best selling products'),
                          )   :
                       SingleChildScrollView(
                        scrollDirection: Axis.horizontal,
                        physics: BouncingScrollPhysics(),
                        child: Row(
                          children: [

                            for(int k=0;k<newbestdata.length;k++)
                            if(newbestdata[k]['type']=='best')
                            InkWell(
                                onTap: (){
                                  Navigator.of(context).push(MaterialPageRoute(builder: (context)=> ProductDetails(data: newbestdata[k],frmwhr: '',packSize: newbestdata[k]["p_other_desc"]['p_pack'])));
                                  keywrds='';
                                  NameController.clear();
                                },
                                child:ProductCard(
                                  ontap:() async{
                                Navigator.of(context).push(MaterialPageRoute(builder: (context)=> ProductDetails(data: newbestdata[k],frmwhr: '',packSize: newbestdata[k]["p_other_desc"]['p_pack'])));

                              },
                              iontap: () async{
                                  if(newbestdata[k]['p_fav'].contains(UlStorage.read('mob'))){
                                     setState(() {
                              newbestdata[k]['p_fav'].remove(UlStorage.read('mob'));
                            });
                                    List res=await PutMethod('showOrders/RHVR5A/${newbestdata[k]['p_barcode']}',
                                    jsonEncode({
                               "type" : "remove",
                               "phone" : UlStorage.read('mob')
                            }));

                                  }
                                  else{
                                       setState(() {
                              newbestdata[k]['p_fav'].add(UlStorage.read('mob'));
                            });
                      List resa=await PutMethod('showOrders/RHVR5A/${newbestdata[k]['p_barcode']}',
                                    jsonEncode({
                               "type" : "add",
                               "phone" : UlStorage.read('mob')
                            }));

                                  }
                              },
                                  ImageUrl: newbestdata[k]['p_img'].length==0 ?
                                  Base64Codec().decode(noimg) :  newbestdata[k]['p_img'][0] ,
                                  productName: newbestdata[k]['p_name'],productPercentage:newbestdata[k]['p_type'] ? '': newbestdata[k]['p_discount'],
                                  icon: newbestdata[k]['p_fav'].contains(UlStorage.read('mob')),
                                    mrp: newbestdata[k]['p_mrp'], brandName: newbestdata[k]['p_brand'],
                                    productRate:
                                  newbestdata[k]['p_type'] ?
                                  newbestdata[k]['p_sellingprice']["0"]['p_price'] :
                                  newbestdata[k]['p_sellingprice'], packSize: newbestdata[k]["p_other_desc"]['p_pack'],)),


                          ],
                        ),
                      ),

                     SizedBox(height:20),

                        ],
                      )
                    ],
                  ),
                ),
                if(ActivePage=='category')
                  Column(
                    children: [
                      Center(
                          child: HeadingPoppins(text: 'Categories')),
                      SizedBox(height: 30),
                      catlist.length==0?
                      Loadingforcategorysingle():

                      catlist[0]['status']=='nok' ? Text('No Data found') :
                      GridView(
                        shrinkWrap: true,
                        scrollDirection: Axis.vertical,
                        physics: BouncingScrollPhysics(),


                        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                            crossAxisCount: 2,
                            crossAxisSpacing: 2,
                            mainAxisSpacing:10,
                            mainAxisExtent: 100
                        ),
                        children: [

                          for(int f=0;f<catlist.length;f++)

                            InkWell(
                              onTap: (){

                                Navigator.of(context).push(MaterialPageRoute(builder: (context)=> cateofproduct(catname: catlist[f]['c_name'],)));
                              },
                              child: CategoryProductCard(
                                  ImageUrl: Base64Codec().decode(catlist[f]['c_img']),
                                  startingRate: '200',
                                  itemsCount: '200',
                                  productName:catlist[f]['c_name']
                              ),
                            ),
                        ],
                      ),


                    ],

                  ),
                if(ActivePage=='profile')
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 12.0),

                    child: MyAccount(),
                  ),
                if(ActivePage=='mycart')
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 12.0),

                    child: MyCart(),
                  ),
                if(ActivePage=='favourite')
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 10),

                    child: MyFavourites(),
                  ),
              ],
            ),
          ),
        ),
           bottomNavigationBar:Container(
             height: 70,
             decoration: BoxDecoration(
               color:Colors.white,
               boxShadow: [
                 BoxShadow(
                   color:  UiColors.Shadow_Clr2,

                   offset: const Offset(
                     0.0,
                     0.0,
                   ),
                   blurRadius: 15.0,
                 ),
               ],
               //      borderRadius: BorderRadius.only(topLeft: Radius.circular(25),topRight: Radius.circular(25))
             ),
             child: Center(
               child: Row(
                 crossAxisAlignment: CrossAxisAlignment.center,
                 mainAxisAlignment: MainAxisAlignment.spaceAround,
                 children: [
                   GestureDetector(
                       onTap: (){
                         setState(() {
                           ActivePage='home';
                         });
                       },
                       child:ActivePage == 'home' ? SvgPicture.asset('assets/homepage/Home_active.svg',   ) : SvgPicture.asset('assets/homepage/Home_inactive.svg',  )),
                   GestureDetector(
                       onTap: (){
                         setState(() {
                           ActivePage='category';
                           keywrds='';
                           NameController.clear();
                         });
                       },
                       child:ActivePage == 'category' ? SvgPicture.asset('assets/homepage/Category_active.svg'  ): SvgPicture.asset('assets/homepage/Category_inactive.svg',   )),
                   GestureDetector(
                       onTap: (){
                         setState(() {
                           ActivePage='mycart';
                           keywrds='';
                           NameController.clear();
                         });
                       },
                       child: ActivePage == 'mycart' ? SvgPicture.asset('assets/homepage/Mycart__active.svg',    ):
                       (StreamBuilder(
                           stream: FirebaseFirestore.instance
                               .collection("unobi_bills").where('b_to',isEqualTo: '${UlStorage.read('mob')}').where('b_status',isEqualTo: 'Open').where('b_alert',isEqualTo: 'Yes').snapshots(),
                           builder: (BuildContext context,
                               AsyncSnapshot<QuerySnapshot> snapshot) {
                             if(!snapshot.hasData){
                               return SvgPicture.asset('assets/homepage/Mycart__inactive.svg',);
                              }
                             else{
                               if(snapshot.data?.docs.length == 0){
                                 return SvgPicture.asset('assets/homepage/Mycart__inactive.svg',);
                               }
                               else{
                                 return SvgPicture.asset('assets/homepage/Mycart__inactive2.svg',);

                               }
                             }
                           }) )
                       ),
                   GestureDetector(
                       onTap: (){
                         setState(() {
                           ActivePage='favourite';
                           keywrds='';
                           NameController.clear();
                         });
                       },
                       child: ActivePage == 'favourite' ? SvgPicture.asset('assets/homepage/Favourite_active.svg',    ): SvgPicture.asset('assets/homepage/Favourite_inactive.svg',   )),
                   GestureDetector(
                       onTap: (){
                         setState(() {
                           ActivePage='profile';
                           keywrds='';
                           NameController.clear();
                         });
                       },
                       child: ActivePage == 'profile' ? SvgPicture.asset('assets/homepage/Profil_active.svg',    ): SvgPicture.asset('assets/homepage/Profil_inactive.svg',   )),

                 ],
               ),
             ),
           )


        ));
  }
  

}
