import 'dart:convert';

import 'package:firebase_auth/firebase_auth.dart';
import 'package:http/http.dart' as http;
import 'package:intl/intl.dart';
import 'package:flutter/material.dart';
import 'package:unilabs/Controllers/ApiCalls.dart';
import 'package:unilabs/Controllers/Constants.dart';
import 'package:unilabs/Models/AppBar_Widgets.dart';
import 'package:unilabs/Models/ProductCard.dart';
import 'package:unilabs/Models/Theme.dart';
import 'package:expandable/expandable.dart';
import 'package:unilabs/Views/Home/BottomNavigation.dart';
import 'package:unilabs/Views/Home/ProductDetails.dart';
import 'package:get_storage/get_storage.dart';
import 'package:unilabs/Views/Home/Sales_Details.dart';
import '../../Models/HeadingText.dart';
import '../../Models/MedButton.dart';
import 'package:flutter_svg/flutter_svg.dart';
import 'package:flutter_spinkit/flutter_spinkit.dart';
import '../../Models/dashedseprator.dart';
import 'BasePage.dart';
var noimg="";
final UlStorage=GetStorage();
class AllProducts_by_UriLink extends StatefulWidget {
  final from;
  const AllProducts_by_UriLink({Key? key,this.from}) : super(key: key);

  @override
  _AllProducts_by_UriLinkState createState() => _AllProducts_by_UriLinkState();
}

class _AllProducts_by_UriLinkState extends State<AllProducts_by_UriLink> {
  int limit=1;
  bool showload=false;
  List products=[];
  AllProductApi() async{
    var token = await FirebaseAuth.instance.currentUser?.getIdToken();

    var response2s = await http.get(Uri.parse(
        "${ConstantsN.baseurl}/addProducts/RHVR5A/${UlStorage.read('mob')}/${limit}"),
        headers: {
          "Accept": "application/json", "authorization": 'Bearer $token'
        }
    );




    setState(() {
      products.addAll(json.decode(response2s.body));
      showload=false;
    });
  }
  String sortby='';
  @override
  void initState() {
    sortby='zta';
    limit=1;
    AllProductApi();
print('initstate called');

    print(products);

    super.initState();

  }
  void dispose() {

    super.dispose();

  }
  @override
  Widget build(BuildContext context) {


    return SafeArea(
        child: Scaffold(
          backgroundColor: Colors.white,
          appBar:AppBar(
            elevation: 0,
            backgroundColor: Colors.white,
            title: Text('Products',style: TextStyle(fontSize: 17.5,fontFamily: 'poppins',color: Colors.black),),
            centerTitle: true,
            leading: IconButton(
              onPressed: (){
                Navigator.pop(context);

              },icon: Icon(Icons.arrow_back_ios,color:Colors.black),),

          ),
          body: SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.all(15.0),
              child: Column(
                children: [

                  products.length==0 ?
                  loadmyorders():
                  (products.length!=0 && products[0]['status']=='ok')  ?
                  Column(
                    children: [
                      GridView(
                          shrinkWrap: true,
                          scrollDirection: Axis.vertical,
                          physics: BouncingScrollPhysics(),


                          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                              crossAxisCount: 2,
                              crossAxisSpacing: 0,
                              mainAxisSpacing:0.0,
                              mainAxisExtent: 290
                          ),
                          children:  [
                            for(int k=0; k<products.length; k++)
                              if(products[k]['status']=='ok')
                                ProductCardSec(
                                  ontap:(){
                                    Navigator.of(context).push(MaterialPageRoute(builder: (context)=> ProductDetails(data: products[k],frmwhr: 'category',packSize: products[k]["p_other_desc"]['p_pack'],)));
                                  },
                                  iontap: () async{    if(products[k]['p_fav'].contains(UlStorage.read('mob'))){
                                    setState(() {
                                      products[k]['p_fav'].remove(UlStorage.read('mob'));
                                    });
                                    List res=await PutMethod('showOrders/RHVR5A/${products[k]['p_barcode']}',
                                        jsonEncode({
                                          "type" : "remove",
                                          "phone" : UlStorage.read('mob')
                                        }));

                                  }
                                  else{
                                    setState(() {
                                      products[k]['p_fav'].add(UlStorage.read('mob'));
                                    });
                                    List resa=await PutMethod('showOrders/RHVR5A/${products[k]['p_barcode']}',
                                        jsonEncode({
                                          "type" : "add",
                                          "phone" : UlStorage.read('mob')
                                        }));

                                  }
                                  },
                                  ImageUrl: products[k]['p_img'].length==0 ?
                                  Base64Codec().decode(noimg) :  products[k]['p_img'][0],
                                  productName: products[k]['p_name'],productPercentage: products[k]['p_type'] ? '': products[k]['p_discount'],
                                  icon: products[k]['p_fav'].contains(UlStorage.read('mob')),
                                  mrp:products[k]['p_mrp'], brandName: products[k]['p_brand'],

                                  productRate:
                                  products[k]['p_type'] ?
                                  products[k]['p_sellingprice']["0"]['p_price'] :
                                  products[k]['p_sellingprice'], packSize: products[k]["p_other_desc"]['p_pack'],
                                ),



                          ]

                      ),
                      SizedBox(height: 15,),
                      if(products[products.length-1]['status']!='nok')
                        Column(
                          children: [
                            showload==false ?
                            UiButton(text: 'Show More',ontap: (){
                              setState(() {
                                limit =limit+10;
                                showload=true;
                                AllProductApi();
                              });
                            },

                            ) :

                            Center(
                                child:Row(
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    children: [
                                      Padding(
                                        padding:EdgeInsets.fromLTRB(0,0,0,0),
                                        child:  SpinKitHourGlass(
                                          color: UiColors.gradient1,size: 30,


                                        ),
                                      ),
                                      Text('  Loading....',style: TextStyle(fontSize: 15,fontFamily: 'poppins',color: Colors.black),),
                                    ]
                                )),
                          ],
                        ),
                      if(products[products.length-1]['status']=='nok')
                        Center(
                            child:Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [

                                  Text('No more products available',style: TextStyle(fontSize: 15,fontFamily: 'poppins',color: Colors.black),),
                                ]
                            )),
                    ],    )

                      :
                  Padding(
                    padding: const EdgeInsets.only(top:8.0),
                    child: Center(child: Text('No Products found')),
                  ),


                ],
              ),
            ),
          ),
          bottomNavigationBar: Bottomnavigation(toScreen: 'category',),
        ));
  }

  sheetopen() {
    showModalBottomSheet<void>(
      // context and builder are
      // required properties in this widget
      context: context,
      builder: (BuildContext context) {
        // we set up a container inside which
        // we create center column and display text

        // Returning SizedBox instead of a Container
        return SizedBox(
          height: 200,
          child: Padding(
            padding: const EdgeInsets.all(8.0),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children:   <Widget>[
                Text('Sort By'),
                SizedBox(height: 10,),
                /*  Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='lth' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);
                      calllowtohigh();

                    },
                    trailing:sortby=='lth' ? Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Price: Low to High',style: TextStyle(color:
                    sortby=='lth' ? Colors.white : Colors.black
                    ),),),
                ),
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='htl' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);
                      callhightolow();
                    },
                    trailing: sortby=='htl' ?Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Price: High to Low',style: TextStyle(color:
                    sortby=='htl' ? Colors.white : Colors.black
                    ),),),
                ),*/
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='atz' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);
                      callabsort();

                    },
                    trailing: sortby=='atz' ?Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Date : Asceding',style: TextStyle(color:
                    sortby=='atz' ? Colors.white : Colors.black
                    ),),),
                ),
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='zta' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);

                      callbasort();
                    },
                    trailing: sortby=='zta' ?Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Date : Descending',style: TextStyle(color:
                    sortby=='zta' ? Colors.white : Colors.black
                    ),),),
                )
              ],
            ),
          ),
        ); },
    );
  }

  callabsort() {
    products.sort((a, b) {
      return a['b_number'].compareTo(b['b_number']);
    });
    setState(() {
      products=products;
      sortby='atz';
    });
  }
  callbasort() {
    products.sort((b, a) {
      return a['b_number'].compareTo(b['b_number']);
    });
    setState(() {
      products=products;
      sortby='zta';
    });
  }
  calllowtohigh() {
    print("callllllll");
    products.sort((a, b) {


      return ( (a['p_type']==false ? num.parse(a['p_sellingprice']):num.parse((a['p_sellingprice']["0"]['p_price'])))
          .compareTo((b['p_type']==false ? num.parse(b['p_sellingprice']):num.parse(b['p_sellingprice']["0"]['p_price']))));

    });
    setState(() {
      products=products;
      sortby='lth';
    });
  }
  callhightolow() {
    products.sort((b, a) {
      return ( (a['p_type']==false ? num.parse(a['p_sellingprice']):num.parse((a['p_sellingprice']["0"]['p_price'])))
          .compareTo((b['p_type']==false ? num.parse(b['p_sellingprice']):num.parse(b['p_sellingprice']["0"]['p_price']))));
    });
    setState(() {
      products=products;
      sortby='htl';
    });
  }


}
