import 'dart:convert';

import 'package:firebase_auth/firebase_auth.dart';
import 'package:http/http.dart' as http;
import 'package:intl/intl.dart';
import 'package:flutter/material.dart';
import 'package:unilabs/Controllers/ApiCalls.dart';
import 'package:unilabs/Controllers/Constants.dart';
import 'package:unilabs/Models/AppBar_Widgets.dart';
import 'package:unilabs/Models/ProductCard.dart';
import 'package:unilabs/Models/Theme.dart';
import 'package:expandable/expandable.dart';
import 'package:unilabs/Views/Home/BottomNavigation.dart';
import 'package:unilabs/Views/Home/ProductDetails.dart';
import 'package:get_storage/get_storage.dart';
import 'package:unilabs/Views/Home/Sales_Details.dart';
import '../../Models/HeadingText.dart';
import '../../Models/MedButton.dart';
import 'package:flutter_svg/flutter_svg.dart';
import 'package:flutter_spinkit/flutter_spinkit.dart';
import '../../Models/dashedseprator.dart';
import 'BasePage.dart';
var noimg="";
final UlStorage=GetStorage();
class TrackingOrders extends StatefulWidget {
  final from;
  const TrackingOrders({Key? key,this.from}) : super(key: key);

  @override
  _TrackingOrdersState createState() => _TrackingOrdersState();
}

class _TrackingOrdersState extends State<TrackingOrders> {
  int limit=1;
  bool showload=false;
  List Track=[];
  catecall() async{
    var token = await FirebaseAuth.instance.currentUser?.getIdToken();

    var response2s = await http.get(Uri.parse(
        "${ConstantsN.baseurl}/showOrders/RHVR5A/${UlStorage.read('mob')}/confirmedorders/${limit}"),
        headers: {
          "Accept": "application/json", "authorization": 'Bearer $token'
        }
    );




    setState(() {
      Track.addAll(json.decode(response2s.body));
      showload=false;
    });
  }
  String sortby='';
  @override
  void initState() {
    sortby='zta';
    limit=1;
    catecall();


    print(Track);

    super.initState();

  }
  void dispose() {

    super.dispose();

  }
  @override
  Widget build(BuildContext context) {
print(Track);
print(limit);
    return SafeArea(
        child: Scaffold(
          backgroundColor: Colors.white,
          appBar:AppBar(
            elevation: 0,
            backgroundColor: Colors.white,
            title: Text('My Orders',style: TextStyle(fontSize: 17.5,fontFamily: 'poppins',color: Colors.black),),
            centerTitle: true,
            leading: IconButton(
              onPressed: (){
                if(widget.from != 'ordersummary')

                  Navigator.pushAndRemoveUntil(
                    context, MaterialPageRoute(builder: (context) => BaseScreen(toScreen: 'profile',)),
                        (Route<dynamic> route) => false,
                  );
                if(widget.from == 'ordersummary')
                  Navigator.pushAndRemoveUntil(
                    context, MaterialPageRoute(builder: (context) => BaseScreen(toScreen: '',)),
                        (Route<dynamic> route) => false,
                  );

              },icon: Icon(Icons.arrow_back_ios,color: Colors.black,),),

          ),
          body: SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.all(15.0),
              child: Column(
                children: [

                  Track.length==0 ?
                  loadmyorders():
                  (Track.length!=0 && Track[0]['status']=='ok')  ?
                  Column(
                    children: [
                      Align(
                        alignment: Alignment.centerRight,
                        child:  IconButton(
                            onPressed: (){
                              sheetopen();
                            },
                            icon: Icon(Icons.filter_list_outlined)),
                      ),
                      for(int k=0; k<Track.length; k++)
                        if(Track[k]['status']=='ok')
                        Padding(
                          padding: const EdgeInsets.only(bottom: 10.0),

                          child: TextButton(
                              onPressed: (){
                                Navigator.of(context).push(MaterialPageRoute(builder: (context)=> BillDetails(invnumber: Track[k]['bill_number'], phone: UlStorage.read('mob'), tt: '0.1' ,)));
                              },
                              child: Container(
                                width: MediaQuery.of(context).size.width*0.94,
                                decoration: BoxDecoration(
                                    boxShadow: [
                                      BoxShadow(
                                        color:   UiColors.Shadow_Clr2  ,
                                        offset: const Offset(
                                          1.0,
                                          3.0,
                                        ),
                                        blurRadius:8.0,
                                      ),
                                    ],
                                    color: Colors.white,
                                    borderRadius: BorderRadius.circular(8)
                                ),
                                child: Padding(
                                  padding: const EdgeInsets.only(left: 15,top: 15,bottom: 12,right: 10),
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.end,
                                    children: [

                                      Row(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                        children: [
                                          Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Row(
                                                children: [
                                                  Text('Bill No: ',style: TextStyle(color: UiColors.primary,fontFamily: 'cera medium',fontSize: 15),),
                                                  Text('${Track[k]['bill_number'].substring(7)}',style: TextStyle(color: Colors.black,fontFamily: 'cera medium',fontSize: 15),),
                                                ],
                                              ),
                                              SizedBox(height: 5),
                                              Row(
                                                children: [
                                                  Text('Status :  ',style: TextStyle(color: UiColors.primary,fontFamily: 'cera medium',fontSize: 15),),
                                                  StatusText(k),
                                                ],
                                              ),
                                              SizedBox(height: 5),
                                          Text('${readTimestamp(Track[k]['b_number'].toString())}',style: TextStyle(fontFamily: 'cera medium',color: Colors.black38,fontSize: 13.5,),textAlign: TextAlign.right,),
                                             SizedBox(height: 5),
                                             ],
                                          ),
                                          Column(
                                            children: [
                                              Container(
                                                decoration: BoxDecoration(

                                                    shape: BoxShape.circle
                                                ),
                                                child: Padding(
                                                    padding: const EdgeInsets.all(8.0),
                                                    child:Column(
                                                      children: [
                                                        if(Track[k]['b_status'] == 'Process')
                                                          SvgPicture.asset('assets/svgs/process.svg',height: 50),

                                                        if(Track[k]['b_status'] == 'Indelivery')
                                                          SvgPicture.asset('assets/svgs/indelivery.svg',height: 50),

                                                        if(Track[k]['b_status'] == 'Packed')
                                                          SvgPicture.asset('assets/svgs/packed.svg',height: 50),

                                                        if(Track[k]['b_status'] == 'Delivered')
                                                          SvgPicture.asset('assets/svgs/delivered.svg',height: 50),

                                                      ],
                                                    )
                                                ),
                                              ),



                                            ],
                                          )
                                        ],
                                      ),
                                      MySeparator(color: UiColors.primary),
                                      SizedBox(height: 8),
                                      Row(
                                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                        crossAxisAlignment: CrossAxisAlignment.end,
                                        children: [
                                          Text('Total Amount ',style: TextStyle(color: UiColors.primary,fontFamily: 'cera',fontSize: 15.5),),
                                          Text('${ConstantsN.currency}${num.parse(Track[k]['b_total']).toStringAsFixed(2)} ',style: TextStyle(color: Colors.black,fontFamily: 'cera medium',fontSize: 17),),
                                        ],
                                      ),

                                    ],
                                  ),
                                ),
                              )
                          ),
                        ),
                      showload==false  ?
                      Column(
                        children: [
                          if(Track.length>9 &&  Track[Track.length-1]['status']=='ok')

                                UiButton(text: 'Load More',ontap: (){
                              setState(() {
                                limit =limit+10;
                                showload=true;
                                catecall();
                              });
                            }, ) ,
                          if(Track[0]['status']!='nok' && Track[Track.length-1]['status']=='nok')
                            Text("No more Bill is there")
                        ],
                      )
                          :

                      Center(
                        child:Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                           Padding(
                             padding:EdgeInsets.fromLTRB(0,0,0,0),
                              child:  SpinKitHourGlass(
                                color: UiColors.gradient1,size: 30,


                              ),
                           ),
                          Text('  Loading....',style: TextStyle(fontSize: 15,fontFamily: 'poppins',color: Colors.black),),
                              ]
                      )),
                    ],)

                      :
                  Padding(
                    padding: const EdgeInsets.only(top:8.0),
                    child: Center(child: Text('No Data found')),
                  ),

                ],
              ),
            ),
          ),
          bottomNavigationBar: Bottomnavigation(toScreen: 'profile',),
        ));
  }
  String readTimestamp(  timestamps) {
    int timestamp=int.parse(timestamps);
    var now = DateTime.now();
    var format = DateFormat('dd - MMM - yyyy hh:mm a');

    var date = DateTime.fromMillisecondsSinceEpoch((timestamp));
    var diff = now.difference(date);
    var time = format.format(date);
    var times = time.toString();
    return times;
  }

   StatusText(k){
   return Column(
     children: [
       if(Track[k]['b_status'] == 'Process')
         Text('${Track[k]['b_status']}',style: TextStyle(fontFamily: 'cera medium',fontSize: 15,color: UiColors.primary),),

       if(Track[k]['b_status'] == 'Packed')
         Text('${Track[k]['b_status']}',style: TextStyle(fontFamily: 'cera medium',fontSize: 15,color: Colors.deepOrange),),

       if(Track[k]['b_status'] == 'Indelivery')
         Text('${Track[k]['b_status']}',style: TextStyle(fontFamily: 'cera medium',fontSize: 15,color: UiColors.indelivery),),

       if(Track[k]['b_status'] == 'Delivered')
         Text('${Track[k]['b_status']}',style: TextStyle(fontFamily: 'cera medium',fontSize: 15,color: UiColors.delivered),),

     ],
   );
  }
  sheetopen() {
    showModalBottomSheet<void>(
      // context and builder are
      // required properties in this widget
      context: context,
      builder: (BuildContext context) {
        // we set up a container inside which
        // we create center column and display text

        // Returning SizedBox instead of a Container
        return SizedBox(
          height: 200,
          child: Padding(
            padding: const EdgeInsets.all(8.0),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children:   <Widget>[
                Text('Sort By'),
                SizedBox(height: 10,),
              /*  Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='lth' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);
                      calllowtohigh();

                    },
                    trailing:sortby=='lth' ? Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Price: Low to High',style: TextStyle(color:
                    sortby=='lth' ? Colors.white : Colors.black
                    ),),),
                ),
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='htl' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);
                      callhightolow();
                    },
                    trailing: sortby=='htl' ?Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Price: High to Low',style: TextStyle(color:
                    sortby=='htl' ? Colors.white : Colors.black
                    ),),),
                ),*/
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='atz' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);
                      callabsort();

                    },
                    trailing: sortby=='atz' ?Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Date : Asceding',style: TextStyle(color:
                    sortby=='atz' ? Colors.white : Colors.black
                    ),),),
                ),
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='zta' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);

                      callbasort();
                    },
                    trailing: sortby=='zta' ?Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Date : Descending',style: TextStyle(color:
                    sortby=='zta' ? Colors.white : Colors.black
                    ),),),
                )
              ],
            ),
          ),
        ); },
    );
  }

  callabsort() {
    Track.sort((a, b) {
      return a['b_number'].compareTo(b['b_number']);
    });
    setState(() {
      Track=Track;
      sortby='atz';
    });
  }
  callbasort() {
    Track.sort((b, a) {
      return a['b_number'].compareTo(b['b_number']);
    });
    setState(() {
      Track=Track;
      sortby='zta';
    });
  }
  calllowtohigh() {
    print("callllllll");
    Track.sort((a, b) {


      return ( (a['p_type']==false ? num.parse(a['p_sellingprice']):num.parse((a['p_sellingprice']["0"]['p_price'])))
          .compareTo((b['p_type']==false ? num.parse(b['p_sellingprice']):num.parse(b['p_sellingprice']["0"]['p_price']))));

    });
    setState(() {
      Track=Track;
      sortby='lth';
    });
  }
  callhightolow() {
    Track.sort((b, a) {
      return ( (a['p_type']==false ? num.parse(a['p_sellingprice']):num.parse((a['p_sellingprice']["0"]['p_price'])))
          .compareTo((b['p_type']==false ? num.parse(b['p_sellingprice']):num.parse(b['p_sellingprice']["0"]['p_price']))));
    });
    setState(() {
      Track=Track;
      sortby='htl';
    });
  }


}
