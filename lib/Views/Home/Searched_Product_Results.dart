...
class SearchedProductResults extends StatefulWidget {
  final data;
  const SearchedProductResults({Key? key,required this.data}) : super(key: key);

  @override
  _SearchedProductResultsState createState() => _SearchedProductResultsState();
}

class _SearchedProductResultsState extends State<SearchedProductResults> {
  List newbestdata=[];
  newandbestcall() async{

    List res=await PutMethod('addProducts/${UlStorage.read('comcode')}/${UlStorage.read('mob')}/searchpage',
        jsonEncode({
          "keyword" :widget.data,
    ...
    return SafeArea(
        child: Scaffold(
          ...
          appBar: AppBar(
            elevation: 0,
            backgroundColor: Colors.white,
            title: Text('Search Results',style: TextStyle(fontSize: 17.5,fontFamily: 'poppins',color: Colors.black),),
            centerTitle: true,
            leading: IconButton(
              onPressed: (){
