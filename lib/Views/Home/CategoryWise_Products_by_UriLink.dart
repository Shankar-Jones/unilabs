import 'dart:convert';
import 'package:flutter/material.dart';
import 'package:unilabs/Controllers/ApiCalls.dart';
import 'package:unilabs/Models/AppBar_Widgets.dart';
import 'package:unilabs/Models/ProductCard.dart';
import 'package:unilabs/Models/Theme.dart'; 
import 'package:unilabs/Views/Home/BottomNavigation.dart';
import 'package:unilabs/Views/Home/ProductDetails.dart';

import 'package:get_storage/get_storage.dart';
final UlStorage=GetStorage();
String sortby='';
var noimg="";

class CategoryWise_Products_by_UriLink extends StatefulWidget {
  final categoryName;
  const CategoryWise_Products_by_UriLink({Key? key, this.categoryName}) : super(key: key);

  @override
  _CategoryWise_Products_by_UriLinkState createState() => _CategoryWise_Products_by_UriLinkState();
}

class _CategoryWise_Products_by_UriLinkState extends State<CategoryWise_Products_by_UriLink> {
  List newbestdata=[];
  newandbestcall() async{
    List respona=await GetMethod('addCategory/RHVR5A/${widget.categoryName}');
    setState(() {
      newbestdata=respona;
    });
  }
  @override
  void initState() {
    print(newbestdata.length);
    print('newbestdata.length');
    print(newbestdata.length);
    newandbestcall();
    super.initState();
    sortby='';
  }
  void dispose() {

    super.dispose();

  }
  @override
  Widget build(BuildContext context) {
print(widget.categoryName.replaceAll(RegExp(' _'),' '));
print(widget.categoryName);
print(newbestdata);
    return SafeArea(
        child: Scaffold(
          backgroundColor: Colors.white,
          appBar: shopappBar(widget.categoryName,context),
          body: SingleChildScrollView(
            child: Padding(
              padding: const EdgeInsets.all(15.0),
              child: Column(
                children: [

                  newbestdata.length==0 ?
                  loadproductfull():
                  (newbestdata.length!=0 && newbestdata[0]['status']=='ok')  ?
                  Column(

                    children: [
                      SizedBox(height: 0,),
                      Align(
                        alignment: Alignment.centerRight,
                        child:  IconButton(
                            onPressed: (){
                              sheetopen();
                            },
                            icon: Icon(Icons.filter_list_outlined)),
                      ),

                      GridView(
                          shrinkWrap: true,
                          scrollDirection: Axis.vertical,
                          physics: BouncingScrollPhysics(),


                          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                              crossAxisCount: 2,
                              crossAxisSpacing: 0,
                              mainAxisSpacing:0,
                              mainAxisExtent: 290
                          ),
                          children:  [
                            for(int k=0; k<newbestdata.length; k++)
                              ProductCardSec(
                                ontap:(){
                                  Navigator.of(context).push(MaterialPageRoute(builder: (context)=> ProductDetails(data: newbestdata[k],frmwhr: 'category',packSize: newbestdata[k]["p_other_desc"]['p_pack'])));
                                },
                                iontap: () async{
                                  if(newbestdata[k]['p_fav'].contains(UlStorage.read('mob'))){
                                    setState(() {
                                      newbestdata[k]['p_fav'].remove(UlStorage.read('mob'));
                                    });
                                    List res=await PutMethod('showOrders/RHVR5A/${newbestdata[k]['p_barcode']}',
                                        jsonEncode({
                                          "type" : "remove",
                                          "phone" : UlStorage.read('mob')
                                        }));

                                  }
                                  else{
                                    setState(() {
                                      newbestdata[k]['p_fav'].add(UlStorage.read('mob'));
                                    });
                                    List resa=await PutMethod('showOrders/RHVR5A/${newbestdata[k]['p_barcode']}',
                                        jsonEncode({
                                          "type" : "add",
                                          "phone" : UlStorage.read('mob')
                                        }));

                                  }
                                },
                                ImageUrl: newbestdata[k]['p_img'].length==0 ?
                                Base64Codec().decode(noimg) :  newbestdata[k]['p_img'][0] ,
                                productName: newbestdata[k]['p_name'],productPercentage:newbestdata[k]['p_type'] ? '': newbestdata[k]['p_discount'],
                                icon: newbestdata[k]['p_fav'].contains(UlStorage.read('mob')),
                                mrp:newbestdata[k]['p_mrp'], brandName: newbestdata[k]['p_brand'],
                                productRate:
                                newbestdata[k]['p_type'] ?
                                newbestdata[k]['p_sellingprice']["0"]['p_price'] :
                                newbestdata[k]['p_sellingprice'], packSize: newbestdata[k]["p_other_desc"]['p_pack'],),



                          ]

                      ),
                    ],
                  )

                      :
                  Padding(
                    padding: const EdgeInsets.only(top:8.0),
                    child: Center(child: Text('No Data found')),
                  ),

                ],
              ),
            ),
          ),
          bottomNavigationBar: Bottomnavigation(toScreen: 'category'),
        ));
  }
  sheetopen() {
    showModalBottomSheet<void>(
      // context and builder are
      // required properties in this widget
      context: context,
      builder: (BuildContext context) {
        // we set up a container inside which
        // we create center column and display text

        // Returning SizedBox instead of a Container
        return SizedBox(
          height: 300,
          child: Padding(
            padding: const EdgeInsets.all(8.0),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children:   <Widget>[
                Text('Sort By'),
                SizedBox(height: 10,),
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='lth' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);
                      calllowtohigh();

                    },
                    trailing:sortby=='lth' ? Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Price: Low to High',style: TextStyle(color:
                    sortby=='lth' ? Colors.white : Colors.black
                    ),),),
                ),
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='htl' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);
                      callhightolow();
                    },
                    trailing: sortby=='htl' ?Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Price: High to Low',style: TextStyle(color:
                    sortby=='htl' ? Colors.white : Colors.black
                    ),),),
                ),
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='atz' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);
                      callabsort();

                    },
                    trailing: sortby=='atz' ?Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Name: A to Z',style: TextStyle(color:
                    sortby=='atz' ? Colors.white : Colors.black
                    ),),),
                ),
                Container(
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8),
                      color:sortby=='zta' ? Colors.blue : Colors.white
                  ),
                  child: ListTile(
                    onTap:(){
                      Navigator.pop(context);

                      callbasort();
                    },
                    trailing: sortby=='zta' ?Icon(Icons.check_circle ,color: Colors.white,):null,
                    title: Text('Name: Z to A',style: TextStyle(color:
                    sortby=='zta' ? Colors.white : Colors.black
                    ),),),
                )
              ],
            ),
          ),
        ); },
    );
  }

  callabsort() {
    newbestdata.sort((a, b) {
      return a['p_name'].toLowerCase().compareTo(b['p_name'].toLowerCase());
    });
    setState(() {
      newbestdata=newbestdata;
      sortby='atz';
    });
  }
  callbasort() {
    newbestdata.sort((b, a) {
      return a['p_name'].toLowerCase().compareTo(b['p_name'].toLowerCase());
    });
    setState(() {
      newbestdata=newbestdata;
      sortby='zta';
    });
  }
  calllowtohigh() {
    print("callllllll");
    newbestdata.sort((a, b) {


      return ( (a['p_type']==false ? num.parse(a['p_sellingprice']):num.parse((a['p_sellingprice']["0"]['p_price'])))
          .compareTo((b['p_type']==false ? num.parse(b['p_sellingprice']):num.parse(b['p_sellingprice']["0"]['p_price']))));

    });
    setState(() {
      newbestdata=newbestdata;
      sortby='lth';
    });
  }
  callhightolow() {
    newbestdata.sort((b, a) {
      return ( (a['p_type']==false ? num.parse(a['p_sellingprice']):num.parse((a['p_sellingprice']["0"]['p_price'])))
          .compareTo((b['p_type']==false ? num.parse(b['p_sellingprice']):num.parse(b['p_sellingprice']["0"]['p_price']))));
    });
    setState(() {
      newbestdata=newbestdata;
      sortby='htl';
    });
  }

}
